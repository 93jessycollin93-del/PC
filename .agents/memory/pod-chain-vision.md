...
**Implemented (2026-07-10):** `src/sas-pod-system/conductor.ts` exports `Conductor.run(chain, input)` — a real (not simulated) sequential runner: hydrates one pod at a time via `PodLifecycleManager`, runs it through the real `runPodTask` executor, always compresses it back down in a `finally` block (even on failure, so no active pod ever leaks), and hands each pod's condensed `summary` forward to the next as context. Ends with a compiled final answer string. Verified via direct script: 3-pod chain completes correctly, and a forced hydration failure halts cleanly with no lingering active instance.
