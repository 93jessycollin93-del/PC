/**
 * Drives the built shell in a real browser against a live jackie-hostd — the
...
if (!process.env.BASE) {
  filesRoot = mkdtempSync(join(tmpdir(), 'jackie-vfs-'));
  hostd = spawn(
    'python3',
    [join(PROJECT, 'image/overlay/opt/jackie-os/bin/jackie-hostd')],
    {
      env: {
        ...process.env,
        JACKIE_STATIC: join(SHELL_DIR, 'dist'),
        JACKIE_FILES: filesRoot,
        JACKIE_PORT: String(PORT),
      },
      stdio: 'ignore',
    },
  );
