...
import { useLongPress, ContextRequest } from '../../src/desktop/useLongPress';

interface HomeScreenProps {
    items: (DesktopItem | null)[];
    onLaunch: (item: DesktopItem) => void;
    /** Right-click / press-and-hold on an icon. */
    onItemContext?: (item: DesktopItem, req: ContextRequest) => void;
    /** Right-click / press-and-hold on empty desktop space. */
    onDesktopContext?: (req: ContextRequest) => void;
}

/**
 * One icon. Split into its own component so each can own a long-press hook
 * (hooks can't be called from inside the render loop), and so all three
 * visual variants share one input path instead of three copies of it.
 */
const DesktopItemView: React.FC<{
    item: DesktopItem;
    themed: boolean;
    iconPack?: any;
    onLaunch: (item: DesktopItem) => void;
    onItemContext?: (item: DesktopItem, req: ContextRequest) => void;
}> = ({ item, themed, iconPack, onLaunch, onItemContext }) => {
    const press = useLongPress(
        (req) => onItemContext?.(item, req),
        !!onItemContext,
    );
    // Suppresses iOS Safari's own press-and-hold callout so ours is the
    // only menu that appears.
    const holdStyle: React.CSSProperties = { WebkitTouchCallout: 'none' };

    if (themed) {
        return (
            <div {...press} style={holdStyle}>
                <PCDesktopIcon item={item} pack={iconPack} onLaunch={onLaunch} />
            </div>
        );
    }
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ items, onLaunch, onItemContext, onDesktopContext }) => {
    ...
    // Empty-space press-and-hold / right-click. Icons carry their own
    // handlers, so a hold that lands on an icon opens the icon's menu.
    const desktopPress = useLongPress(
        (req) => onDesktopContext?.(req),
        !!onDesktopContext,
    );
    return (
            ...
            {items.map((item, index) => {
                ...
                return (
                    <DesktopItemView
                        ...
                        item={item}
                        themed={themed}
                        iconPack={pcTheme?.theme.iconPack}
                        onLaunch={onLaunch}
